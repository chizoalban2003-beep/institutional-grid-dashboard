...
def exam_inputs():
    ...
    against it (v6 bug) penalized fast channels (sine/cosine/lorenz)
    that drift from their ffill while flattering slow ones (decay/step/
    sigmoid). Phase-1's masked_r2 grades against Yte (true values).
    """
...
ARMS = ["ctrl", "iso", "frz"]
FROZEN_CORE_KEYS = ["gru.weight_hh_l0", "gru.bias_ih_l0", "gru.bias_hh_l0",
                    "decode_cell.weight_hh", "decode_cell.bias_ih",
                    "decode_cell.bias_hh", "scorer.weight", "scorer.bias",
                    "cell_block.weight", "cell_block.bias"]


def clinical_columns_zeroed_model(model):
    """Return a model wrapper whose exam forward zeroes the clinical input
    columns of the GRU + decode input projections (mechanism-a isolation).

    The model is a plain MathSchoolGrid; we monkeypatch the GRU input
    projection temporarily at exam time. Simplest faithful approach:
    build a shallow copy with the clinical columns zeroed, run exam on it.
    """
    import copy
    m2 = copy.deepcopy(model)
    with torch.no_grad():
        m2.gru.weight_ih_l0.data[:, CLINICAL_SLOT:] = 0.0
        m2.decode_cell.weight_ih.data[:, CLINICAL_SLOT:] = 0.0
        m2.decode_cell.weight_ih.data[:, D_IN + 64 + K_MATH:] = 0.0
    return m2


def run_arm(arm, Xtr, Ytr, Mtr, Xte, Yte, Mte, t0):
    """One autopsy arm: train the same way, then test exam under the arm's
    hypothesis. ctrl = plain v7 repro; iso = mechanism (a): exam forward
    with clinical input columns zeroed (isolates exam-time pollution from
    learned clinical columns); frz = mechanism (b): shared recurrent core
    FROZEN during clinical training (isolates transition-matrix drift)."""
    print(f"\n===== ARM {arm} =====", flush=True)
    ...
    if arm == "frz":
        n_frozen = 0
        for k, p in model.named_parameters():
            if k in FROZEN_CORE_KEYS:
                p.requires_grad = False
                n_frozen += 1
        print(f"  [frz] shared recurrent core frozen: {n_frozen} tensors "
              f"(weight_hh/scorer/cell_block)", flush=True)
    opt = torch.optim.Adam(
        [{"params": [p for k, p in model.named_parameters()
                     if p.requires_grad and k in TRANSFER_KEYS],
          "lr": LR_TRANSFER},
         {"params": [p for k, p in model.named_parameters()
                     if p.requires_grad and k not in TRANSFER_KEYS],
          "lr": LR_NEW}])
    ...
    iso_r2 = None
    if arm == "iso":
        iso_model = clinical_columns_zeroed_model(model)
        iso_r2 = exam_r2(iso_model, Xex, Yex, Mex)
        print("  [iso] exam with clinical columns zeroed: "
              + " ".join(f"{k} {v:.3f}" for k, v in iso_r2.items()), flush=True)
    ...
    with open(f"/kaggle/working/autopsy_{arm}.json", "w") as f:
        json.dump(report, f, indent=2)
    print(f"[arm {arm}] {report['seconds']}s | verdict {report['verdict']}",
          flush=True)
    return report
...
def main():
    ...
    reports = [run_arm(a, Xtr, Ytr, Mtr, Xte, Yte, Mte, t0) for a in ARMS]
    summary = {r["arm"]: {"clinical_r2": r["clinical_masked_r2"],
                          "worst_exam": min(r["exam_r2_final"].values()),
                          "worst_exam_iso": (min(r["exam_r2_iso"].values())
                                             if r.get("exam_r2_iso") else None),
                          "verdict": r["verdict"]} for r in reports}
    print("\n=== AUTOPSY SUMMARY ===", flush=True)
    for arm, s in summary.items():
        print(f"  {arm}: clinical {s['clinical_r2']:.4f} | worst exam "
              f"{s['worst_exam']:.3f}" + (f" | iso {s['worst_exam_iso']:.3f}"
                                          if s["worst_exam_iso"] else "")
              + f" | {s['verdict']}", flush=True)
    with open("/kaggle/working/autopsy_fleet.json", "w") as f:
