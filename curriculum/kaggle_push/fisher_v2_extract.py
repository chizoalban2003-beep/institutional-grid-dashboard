...
def main():
    ...
    print("[1/4] Phase-2 model + transfer (exact v8 init)...", flush=True)
    ...
    model.eval()
    print(f"  transfer OK: {len(copied)} full + {len(partial_copied)} column",
          flush=True)

    print("[2/4] certified exam windows...", flush=True)
    X18, Y, M = exam_windows(N_BATCHES * BATCH, SEED + 2)
    n = X18.shape[0]
    print(f"  {n} windows", flush=True)

    print("[3/4] full-surface Fisher accumulation...", flush=True)
    F = {k: torch.zeros_like(p) for k, p in model.named_parameters()}
    for bi in range(N_BATCHES):
        sl = slice(bi * BATCH, (bi + 1) * BATCH)
        grid = embed_math_block(X18[sl])
        xb = torch.tensor(grid, dtype=torch.float32)
        yb = torch.tensor(Y[sl], dtype=torch.float32)
        mb = torch.tensor(M[sl], dtype=torch.float32)
        pred = model(xb)
        pred_k = pred[:, :, :K_MATH]
        dm = 1.0 - mb
        loss = ((pred_k - yb) ** 2 * dm).sum() / max(dm.sum(), 1)
        loss.backward()
    ...
    with open("/kaggle/working/fisher_v2_report.json", "w") as f:
    ...
    print(f"[done] {report['seconds']}s — fisher_v2.npz + "
          f"fisher_v2_report.json", flush=True)
