...
# ---------------------------- main (meta-cognition: trans vs rand)
...
def run_arm(arm, Xtr, Ytr, Mtr, Xte, Yte, Mte, t0):
    """trans = v8 repro (transfer + live anchor); rand = no transfer, no
    anchor. Both log the clinical R2 curve every 5 epochs — the curve
    (epochs-to-0.90) is the positive-scaffolding metric."""
    print(f"\n===== ARM {arm} =====", flush=True)
    torch.manual_seed(SEED)
    np.random.seed(SEED)

    model = MathSchoolGrid(D_IN, HIDDEN, N_CELLS, K_ACTIVE, K_SUBJECTS)
    copied, partial_copied, skipped = [], [], []
    if arm == "trans":
    ...
    if r2_final:
        print("  exam " + " ".join(f"{k} {v:.3f}"
                                   for k, v in r2_final.items()), flush=True)
    print("  VERDICT:", "PASS" if ok else "FAIL", flush=True)

    torch.save(model.state_dict(), f"/kaggle/working/math2clinic_{arm}.pt")
    ...
    with open(f"/kaggle/working/phase2_{arm}.json", "w") as f:
        json.dump(report, f, indent=2)
    print(f"[arm {arm}] {report['seconds']}s | epochs-to-0.90 "
