...
# EWC v2 "TRUST THE SPRING" test: does the full-surface Fisher alone
# arbitrate plasticity, or is the 1e-5 priors LR group load-bearing?
# Fixed lam=10 (the passing arm) x two LR regimes, curve-logged.
LAM_FIXED = 10.0                      # the v2 passing tension
LR_ARMS = ["slow", "fast"]            # slow=v8 param groups; fast=uniform 1e-4
...
def load_fisher_normalized():
    """Load fisher_v2.npz (FULL surface, Phase-2 shapes, no zeroing).

    v2 F is computed on the Phase-2 model at transferred init over ALL
    named parameters (clinical columns + weight_hh included) — the
    co-optation autopsy proved math knowledge lives everywhere, so the
    Cost slice must be measured everywhere. Normalize mean-1 globally
    so relative importance survives and lambda is scale-free.
    """
    ckpt_dir = discover_input(FISHER_DATASET)
    f_raw = np.load(os.path.join(ckpt_dir, FISHER_FILE))
    f2 = {k: np.asarray(f_raw[k], dtype=np.float64)
          for k in f_raw.files}
    ...
    f2 = {k: torch.tensor(v / mean, dtype=torch.float32)
          for k, v in f2.items()}
    print(f"[ewc] full-surface fisher loaded {len(f2)} keys, "
          f"global mean normalized to 1 (raw mean {mean:.2e})", flush=True)
    return f2


def anchor_grad_norm(model, Xex, Yex, Mex):
    """Measure the live anchor's gradient scale on the shared core.

    The v8 anchor (LAMBDA_MATH=0.01 * exam MSE) is the O(N) gold
    standard; lambda in the true-economy sweep must make the EWC
    penalty's gradient comparable. This probe computes the RMS
    gradient magnitude the anchor would apply to the protected
    params, so lambda can be expressed in anchor-equivalent units.
    """
    loss = exam_loss(model, Xex[:128], Yex[:128], Mex[:128])
    loss.backward()
    norms = [p.grad.detach().abs().mean().item()
             for p in model.parameters() if p.grad is not None]
    model.zero_grad(set_to_none=True)
    anchor_norm = float(np.mean(norms)) if norms else 0.0
    print(f"[ewc] live-anchor gradient scale (mean|g| over params): "
          f"{anchor_norm:.3e} — lambda sweep is in these units", flush=True)
    return anchor_norm
...
def clinical_r2_curve(model, Xte, Yte, Mte):
    """Global-mean masked R2 over clinical heads only (Phase-1 semantics)."""
    with torch.no_grad():
        pred = model(Xte)
    dm = 1.0 - Mte[:, :, HEAD_OFFSET_CLINICAL:]
    num = ((pred[:, :, HEAD_OFFSET_CLINICAL:] - Yte[:, :, HEAD_OFFSET_CLINICAL:])
           ** 2 * dm).sum()
    den = ((Yte[:, :, HEAD_OFFSET_CLINICAL:]
            - Yte[:, :, HEAD_OFFSET_CLINICAL:].mean(dim=(0, 1), keepdim=True))
           ** 2 * dm).sum()
    return float(1.0 - num / max(den, 1e-9))


def run_arm(arm, Xtr, Ytr, Mtr, Xte, Yte, Mte, f2, anchor_norm, t0):
    """Trust-the-Spring arm: lam=10 fixed, anchor OFF; slow = v8 param
    groups (priors 1e-5), fast = uniform 1e-4 (Fisher is the SOLE
    plasticity arbiter). Clinical curve logged every 5 epochs."""
    tag = f"{arm}"
    print(f"\n===== ARM {tag} ===== (lam {LAM_FIXED:g} x anchor-equiv, "
          f"anchor OFF, {'slow' if arm == 'slow' else 'UNIFORM 1e-4'})",
          flush=True)
    ...
    print(f"  {n_par:,} params | arm {arm} | lam {LAM_FIXED:g}", flush=True)
    ...
    for ep in range(N_EPOCHS):
        ...
        for i in range(n_batches):
            ...
            ewc = ewc_penalty(model, f2, lam_abs)
            loss = loss + ewc
    ...
    epochs_to_90 = next((c["epoch"] for c in curve
                         if c["clinical_r2"] >= 0.90), None)
    report = {
        "arm": tag,
        "lambda": LAM_FIXED,
        "lambda_abs": lam_abs,
    ...
    print(f"[arm {tag}] {report['seconds']}s | epochs-to-0.90 "
          f"{epochs_to_90} | verdict {report['verdict']}", flush=True)
    return report
...
def main():
    ...
    print("[2/5] fisher + anchor calibration...", flush=True)
    f2 = load_fisher_normalized()
    Xex, Yex, Mex = exam_inputs()
    probe_model = MathSchoolGrid(D_IN, HIDDEN, N_CELLS, K_ACTIVE, K_SUBJECTS)
    ckpt_dir = discover_input("math-school-phase-1-checkpoint")
    load_transfer(os.path.join(ckpt_dir, "math_school_s42.pt"), probe_model)
    probe_model.train()
    anchor_norm = anchor_grad_norm(probe_model, Xex, Yex, Mex)
    ...
    reports = []
    for arm in LR_ARMS:
        reports.append(run_arm(arm, Xtr, Ytr, Mtr, Xte, Yte, Mte, f2,
                               anchor_norm, t0))
    ...
    print("\n=== TRUST-THE-SPRING SUMMARY ===", flush=True)
    print("  baselines: trans(anchor) 15ep/0.988/0.985 | ranka 5ep/0.995/0.917 "
          "| rand 5ep/0.995/n.a. | v2 lam10 40ep/0.988/0.823", flush=True)
    for arm, s in summary.items():
        print(f"  {arm}: epochs-to-0.90 {s['epochs_to_0.90']} | clinical "
              f"{s['clinical_r2']:.4f} | worst exam {s['worst_exam']:.3f} "
              f"| {s['verdict']}", flush=True)
    with open("/kaggle/working/ewc_lr_fleet.json", "w") as f:
        json.dump({"anchor_norm": anchor_norm,
                   "summary": summary, "reports": reports}, f, indent=2)
