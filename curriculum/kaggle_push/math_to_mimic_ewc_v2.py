...
# EWC v2 true-economy: anchor OFF, full-surface Fisher, calibrated lambda.
...
def run_arm(lam, Xtr, Ytr, Mtr, Xte, Yte, Mte, f2, anchor_norm, t0):
    """One lambda arm: transfer init + train + eval (independent model).

    TRUE-ECONOMY (v2): the live exam anchor is OFF in every arm. lam is
    expressed in anchor-equivalent units: the EWC penalty gradient is
    scaled so that lam=1 reproduces the live anchor's gradient scale.
    """
    tag = f"lam{lam:g}"
    print(f"\n===== ARM {tag} ===== (anchor OFF; lam {lam:g} x "
          f"anchor-equivalent)", flush=True)
    ...
    for ep in range(N_EPOCHS):
        ...
        for i in range(n_batches):
            ...
            if not ANCHOR_OFF and LAMBDA_MATH > 0:
...
def main():
    ...
    for lam in LAM_SWEEP:
        reports.append(run_arm(lam, Xtr, Ytr, Mtr, Xte, Yte, Mte, f2,
                               anchor_norm, t0))
    ...
    print("\n=== TRUE-ECONOMY FLEET SUMMARY ===", flush=True)
    print("  baselines: trans (anchor) 15ep/0.988/0.985 | "
          "ranka (no transfer) 5ep/0.995/0.917", flush=True)
